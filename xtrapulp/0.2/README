See below for building xtrapulp.
...
mkdir build
cd build
export Torch_DIR=<path to torch cmake>
    in my case, export Torch_DIR=/usr/common/software/pytorch/1.7.1/lib/python3.8/site-packages/torch/share/cmake/Torch/
cmake ..
cd ..
python save_torch_graph.py --dataset <ogbn dataset, e.g. "ogbn-papers100M">
cd build
./graph_to_binary <path to stored ogb graph, e.g. "../ogbn-papers100M.pt">
cd ..
mpirun -np <nprocs> ./xtrapulp ogbn-papers100M.pt.bin 1000 (see below for xtrapulp flags)
```
